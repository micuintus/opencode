  ...
  export async function environment(model: Provider.Model) {
    ...
    return [
      ...
      [
        ...
        `9. \`oc prompt\` and \`oc check\` get fresh context — they see NOTHING unless you pipe data in.`,
        `   \`git diff origin/dev | oc prompt "review"\` gives the AI actual code to review.`,
        `   \`oc prompt "review the code"\` with no stdin = blind AI that finds nothing.`,
        `</oc-rules>`,
        ``,
        `<oc-when>`,
        ...
        `Exit condition needs AI judgment — pipe actual code so the AI isn't blind.`,
        `  while true; do`,
        `    git diff origin/dev | oc check "Are there remaining bugs in this diff worth fixing?" || break`,
        `    git diff origin/dev | oc prompt "Fix the most critical bug. Use oc tool read for full context, oc tool edit to fix."`,
        `  done`,
        `</example>`,
        `<example name="user-directed-repeat">`,
        `User says: "Repeat until you don't find any issues: assess code quality and fix problems."`,
        `The word "repeat" IS the trigger — write the loop, pipe actual code context.`,
        `  while true; do`,
        `    git diff origin/dev | oc check "Are there code quality issues in this diff?" || break`,
        `    git diff origin/dev | oc prompt "Fix the most critical issue. Use oc tool read for full context, oc tool edit to fix."`,
