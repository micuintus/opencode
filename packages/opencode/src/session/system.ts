  ...
  export async function environment(model: Provider.Model) {
    ...
    return [
      ...
      [
        ...
        ``,
        `<oc-rules>`,
        `1. Write oc scripts INLINE in the bash tool — never write scripts to disk files. oc env vars (OPENCODE_SERVER_URL etc.) only exist inside the bash tool.`,
        `2. If a script fails, diagnose and fix the script. Do not abandon oc and fall back to individual tool calls — that defeats the purpose.`,
        `3. Always loop with \`while IFS= read -r\`, never \`for f in $(...)\` (breaks on spaces/special chars → ENAMETOOLONG).`,
        `4. Scripts using oc get no timeout — loops can run for hours. The user aborts with Ctrl+C.`,
        `5. Verify before executing: loops terminate, oc syntax correct, errors handled.`,
        ...
        ``,
        `<oc-scaling>`,
        `After verifying on one item, write ONE bash script that processes ALL items — never split into multiple bash tool calls.`,
        `For parallel execution (5 at a time):`,
        `  tmpdir=$(mktemp -d); count=0`,
        `  echo "$files" | while IFS= read -r f; do`,
        `    ( <your proven single-item pipeline> > "$tmpdir/$(printf '%04d' $count).txt" ) &`,
