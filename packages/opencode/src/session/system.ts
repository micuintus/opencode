  ...
  export async function environment(model: Provider.Model) {
    ...
    return [
      [
        ...
        `</env>`,
        `<critical-rule>`,
        `When the user says "repeat", "iterate", "keep going until", or "until done": write an oc bash script with a while loop as your FIRST action.`,
        `Do NOT start by manually reading files. Pattern: while assessment=$(oc check "criteria"); do echo "$assessment" | oc prompt "fix"; oc status "done"; done`,
        `Pass the user's SPECIFIC criteria into the oc check string. For planning use oc todo add (NOT markdown checklists).`,
        `</critical-rule>`,
      ].join("\n"),
      [
        `<oc-scripting>`,
        ...
        ``,
        `<oc-when>`,
        `Consider writing an oc script when the user's request involves:`,
        `a) Iterative work toward a verified outcome — fix/retry until tests pass, keep improving until clean.`,
        `   Approach: while-capture loop with \`oc check\` + \`oc prompt\`. This is a top-level orchestration pattern.`,
        `   If your task was delegated (via oc check/prompt), the caller handles iteration — complete your task directly.`,
        ...
        `  oc check "question"                 — grep pattern: assessment → stdout, boolean → exit code 0/1`,
        ...
        `  done`,
        `</example>`,
        `<example name="while-capture">`,
        `User asks: "Keep fixing security issues until the API is clean." (or any repeat/iterate/until request)`,
        `While-capture pattern (like \`while read\`): oc check assesses, findings flow to oc prompt, exit code controls loop.`,
        `Pass the user's SPECIFIC criteria into the oc check string — don't use generic questions.`,
        `  while assessment=$(oc check "Are there security vulnerabilities in the API? Check: auth, injection, rate limiting."); do`,
        `    echo "$assessment" | oc prompt "Fix the most critical vulnerability from the assessment above."`,
