  ...
  export async function environment(model: Provider.Model) {
    ...
    return [
      ...
      [
        ...
        `8. If you plan with a todo list, execute it with a while loop — never step through todos manually one call`,
        `   at a time. Build the list, then drain it: \`while true; do oc check "pending todos?" || break;`,
        ...
        `The word "repeat" IS the trigger — write the loop immediately, do NOT make a todo list.`,
        `  while true; do`,
        `    oc check "Are there remaining code quality issues worth fixing?" || break`,
        `    oc prompt "Find the most important remaining issue and fix it with oc tool edit."`,
