  ...
  export async function environment(model: Provider.Model, opts?: { isChild?: boolean }) {
    ...
    return [
      ...
      ...(opts?.isChild ? [] : [[
        ...
        `While-capture pattern: oc check assesses (stdout), exit code controls loop, findings flow to oc prompt.`,
        `  while assessment=$(oc check "Are there remaining bugs worth fixing? Use tools to investigate."); do`,
        `    echo "$assessment" | oc prompt "Fix the most critical bug from the assessment above."`,
        `  done`,
        `</example>`,
        `<example name="user-directed-repeat">`,
        `User says: "Repeat until you don't find any issues: assess code quality and fix problems."`,
        `While-capture: oc check investigates ONCE, findings pipe to oc prompt — no double investigation.`,
        `  while assessment=$(oc check "Assess code quality on this branch vs origin/dev. Check: standards, bugs, architecture."); do`,
        `    echo "$assessment" | oc prompt "Fix the most critical issue from the assessment above."`,
